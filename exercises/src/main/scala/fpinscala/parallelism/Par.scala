...
object Par {
  ...
  //implicit def toParOps[A](p: Par[A]): ParOps[A] = new ParOps(p)
    ...
    def sequence[A](ps: List[Par[A]]): Par[List[A]] = ps.foldRight(unit[List[A]](List.empty)) {
      (parA,parListA) => map2(parA,parListA){(a,al) => a::al}
    }
  ...
  def parFilter[A](as: List[A])(f: A => Boolean): Par[List[A]] = {
    map(sequence(as.map(asyncF(a => if(f(a)) Some(a) else None))))(_.flatten)
  }
