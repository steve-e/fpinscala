...
object Candy {
  def simulateMachine(inputs: List[Input]): State[Machine, (Int, Int)] = toState(inputs).flatMap { case _ => get.map { case s => (s.coins, s.candies)}
  }

  def toState(inputs: List[Input]): State[Machine, List[Unit]] = {
    sequence(toStates(inputs))
  }

  def toStates(inputs: List[Input]): List[State[Machine, Unit]] = {
    inputs.map(i => modify((s: Machine) => (i, s) match {
